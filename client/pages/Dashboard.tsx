      ...
      <div className="p-6 relative">
        ...
        >
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 className="text-3xl font-bold text-navy-950">Welcome back, John!</h2>
              <p className="text-navy-600 mt-1">You have {notes.length} notes in your collection</p>
            </div>
          ...
          >
{[
              { title: 'Total Notes', value: notes.length.toString(), icon: FileText, color: 'from-blue-500 to-blue-600', bgColor: 'bg-blue-500/10' },
              { title: 'Categories', value: [...new Set(notes.map(n => n.category))].length.toString(), icon: Grid, color: 'from-green-500 to-green-600', bgColor: 'bg-green-500/10' },
              { title: 'Shared', value: notes.filter(n => n.isShared).length.toString(), icon: Share2, color: 'from-purple-500 to-purple-600', bgColor: 'bg-purple-500/10' },
              { title: 'This Week', value: notes.filter(n => {
                const noteDate = new Date(n.createdAt);
                const weekAgo = new Date();
                weekAgo.setDate(weekAgo.getDate() - 7);
                return noteDate >= weekAgo;
              }).length.toString(), icon: Calendar, color: 'from-orange-500 to-orange-600', bgColor: 'bg-orange-500/10' }
            ].map((stat, index) => (
              <motion.div
                key={stat.title}
                initial={{ opacity: 0, y: 20 }}
                ...
                <Card className="relative bg-white/10 backdrop-blur-xl border border-white/20 hover:border-gold-400/30 transition-all duration-300 overflow-hidden">
                  ...
                  <CardContent className="relative p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-navy-200/80 mb-1">{stat.title}</p>
                ...
                >
                  ...
                  <CardContent className="relative">
                    ...
                    <div className="flex flex-wrap gap-1 mb-4">
                      {note.tags.map(tag => (
                        <Badge key={tag} variant="outline" className="text-xs bg-white/5 border-white/20 text-white/70 hover:bg-white/10">
                          {tag}
                        </Badge>
                      ))}
                    </div>
          ...
          {filteredNotes.length === 0 && (
            ...
            >
              ...
              <p className="text-navy-500 mb-6">Try adjusting your search or create a new note</p>
              <Button 
                onClick={handleCreateNote}
